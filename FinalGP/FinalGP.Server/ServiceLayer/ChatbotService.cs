...
namespace FinalGP.Services
{
	...
	public class ChatbotService : IChatbotService
	{
        ...
        private readonly List<(string question, string answer)> _faqData;
        public ChatbotService(ESHContext context)
		{
			_context = context;
            _faqData = new List<(string question, string answer)>
    {
        ("ما هي منصة سكن الطلاب؟", "منصة متخصصة في مساعدة الطلاب للعثور على سكن مناسب بناءً على تفضيلاتهم وميزانيتهم."),
        ("كيف يمكنني التسجيل في المنصة؟", "يمكنك التسجيل كطالب أو مالك سكن عبر النموذج الإلكتروني، مع تقديم الوثائق المطلوبة."),
        ("هل الخدمة مجانية؟", "نعم، الخدمة مجانية للطلاب، بينما قد توجد رسوم رمزية لمالكي السكنات لنشر الإعلانات المميزة."),
        ("كيف أبحث عن سكن يناسب ميزانيتي؟", "أخبرنا بميزانيتك وسنساعدك في العثور على خيارات مناسبة، مثل: 'أبحث عن سكن بـ 1500 جنيه'."),
        ("كيف أعرف أن السكن آمن؟", "جميع السكنات تمر بمراجعة إدارية، كما يمكنك رؤية تقييمات الطلاب السابقين."),
        ("ما هي طرق الدفع المتاحة؟", "نقبل الدفع نقدًا عند الوصول، أو عبر المحفظة الإلكترونية."),
        ("هل يمكنني إلغاء الحجز؟", "نعم، وفقًا لسياسة الإلغاء المحددة من قبل مالك السكن."),
        ("ما هي المرافق المتاحة عادةً في السكنات؟", "واي فاي - تكييف - غسالة - ثلاجة - ديب فريزر - مطبخ مشترك/خاص."),
        ("كيف أطلب سكنًا بمرافق محددة؟", "اكتب: 'أريد سكنًا بواي فاي وتكييف' وسنعرض الخيارات المتاحة."),
        ("كيف أقيّم السكن بعد الإقامة؟", "يمكنك إضافة تقييم من صفحة تفاصيل السكن، من 1 إلى 5 نجوم."),
        ("ماذا أفعل إذا واجهت مشكلة في السكن؟", "يمكنك إبلاغنا عبر نموذج الإبلاغ عن المشكلات أو عمل تقرير، وسنتواصل مع المالك لحلها.")
    };
        }
        // Inside ChatbotService class
        private string GetFAQAnswer(string message)
        {
            // Normalize the input message for better matching
            var normalizedMessage = message.ToLower().Trim();

            foreach (var faqItem in _faqData)
            {
                // Use a simple Contains check for now, but for more complex FAQs,
                // you might need more advanced NLP techniques or regex.
                // Consider variations of the question.
                if (normalizedMessage.Contains(faqItem.question.ToLower().Replace("؟", "").Replace("?", "")))
                {
                    return faqItem.answer;
                }
            }
            return null; // No FAQ answer found
        }
        ...
        public async Task<string> ProcessMessageAsync(string message)
		{
            ...
            // 1. Check for direct FAQ matches FIRST
            var faqAnswer = GetFAQAnswer(message);
            if (faqAnswer != null)
            {
                return faqAnswer;
            }
...
{ "Gym", "صالة رياضية" }
            ...
            var facilityMappings = new Dictionary<string, string>
    {
        ...
        { "جيم", "صالة رياضية" }
            ...
            {
                if (message.ToLower().Contains(mapping.Key.ToLower()))
                {
